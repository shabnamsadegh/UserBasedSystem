...
public partial class FileManager_Funcations : System.Web.UI.Page
{
   ...
   private void FilesULLI(string DirPath)
   {
       ...
       foreach (string Dir in Directorys)
       {
           ...
           int FileType;
           HTML += " <input class='btn' style='float:right;' onclick=OpenFile('" + "2" + "','" + (DirPath + "/" + System.IO.Path.GetFileName(Dir)).Replace(" ", "%20").Replace("//", "/") + "') id='Button1' type='button' value='باز کردن' /> ";
          // HTML += " <input class='btn' style='float:right;' onclick=InsertFile('" + (DirPath + "/" + System.IO.Path.GetFileName(Dir)).Replace(" ", "%20") + "') id='Button2' type='button'  value='انتخاب عکس برای خبر' /> ";
       ...
       var NFolderNames = FolderNames.Split(',');
        ...
        foreach (string FolderName  in NFolderNames)
        {
            if (!string.IsNullOrEmpty(FolderName))
            {
              ...
              //  string a = System.IO.Directory.GetDirectories(Server.MapPath(FolderName)).ToString();
               ...
               if (System.IO.File.Exists(Server.MapPath(FolderName)))
                {
                    String RelativePath = FolderName.Replace(Request.ServerVariables["APPL_PHYSICAL_PATH"], String.Empty);
                    RelativePath = RelativePath.Replace("//", "/");
                    RelativePath = "/PR/FileManager/" + RelativePath;
                    string ifFileInUse = database.CheckIfFileInUse(RelativePath);
                    if (ifFileInUse == "")
                    {
                        System.IO.File.Delete(Server.MapPath(FolderName));
                    }
                    else output(ifFileInUse);
                }
            }
        }
    ...
    private void DeleteFolder(string FolderPath)
    {
       ...
       string[] files =  System.IO.Directory.GetFiles(FolderPath);
        ...
        foreach (string file in files)
       {
           String RelativePath = file.Replace(Request.ServerVariables["APPL_PHYSICAL_PATH"], String.Empty);
           RelativePath = RelativePath.Replace("\\", "/");
           RelativePath = "/" + RelativePath;

           if (database.CheckIfFileInUse(RelativePath) == "")
           {
